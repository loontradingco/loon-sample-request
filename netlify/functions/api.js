    ...
    <body>
      <div class="container">
        ...
        <div class="content">
          ...
          <h2>Wines Requested (${data.wines?.reduce((sum, w) => sum + (w.quantity || 1), 0)} bottles)</h2>
          <div class="wine-list">
            <ul>
              ${data.wines?.map(w => {
                const qty = w.quantity || 1;
                const details = [w.vintage, w.color].filter(Boolean).join(' ');
                const fullName = details ? `${w.name} (${details})` : w.name;
                return `<li><strong>${fullName}</strong> - ${w.producer}${qty > 1 ? ` <span style="color:#666;">(×${qty})</span>` : ''}</li>`;
              }).join('')}
            </ul>
          </div>
...
// Uses free ip-api.com - no signup needed
// ============================================
async function getLocationFromIP(ip) {
  ...
  try {
    // Free ip-api.com service - no API key needed, 45 requests/min limit
...
async function createSampleRequestInNotion(data) {
  ...
  console.log(`Creating ${producers.length} sample request entries for ${producers.join(', ')} (${totalBottles} total bottles)`);
  ...
  for (const producer of producers) {
    ...
    // Calculate total bottles for this producer
    const producerBottles = producerWines.reduce((sum, w) => sum + (w.quantity || 1), 0);
    
    // Format wines with quantity
    const winesShort = producerWines.map(w => {
      const qty = w.quantity || 1;
      const details = [w.vintage, w.color].filter(Boolean).join(' ');
      const wineName = details ? `${w.name} (${details})` : w.name;
      return qty > 1 ? `${wineName} x${qty}` : wineName;
    }).join(', ');
    
    const winesDetailed = producerWines.map(w => {
      const qty = w.quantity || 1;
      const details = [w.vintage, w.color].filter(Boolean).join(' ');
      const wineName = details ? `${w.name} (${details})` : w.name;
      return qty > 1 ? `• ${wineName} - Qty: ${qty}` : `• ${wineName}`;
    }).join('\n');
    ...
    const properties = {
      ...
      'Wines Count': { number: producerBottles },
    ...
    // Create Sample Line Items for each wine in this producer group
    const SAMPLE_LINE_ITEMS_DB_ID = process.env.SAMPLE_LINE_ITEMS_DB_ID || '2d6a824fc71f80559c38d6280bf0e2a4';
    const sampleRequestId = response.id;
    
    console.log(`Sample Request created with ID: ${sampleRequestId}`);
    
    for (const wine of producerWines) {
      const qty = wine.quantity || 1;
      const details = [wine.color, wine.vintage].filter(Boolean).join(' ');
      const lineItemName = `${submissionDate} ${wine.name} ${details} ${qty}`.trim();
      
      try {
        // The property name is "Sample Request" (singular) - relates to Sample Requests database
        const lineItemProperties = {
          'Name': { title: [{ text: { content: lineItemName } }] },
          'Sample Request': { relation: [{ id: sampleRequestId }] },
          'Quantity': { number: qty }
        };
        
        console.log(`Creating line item "${lineItemName}" with Sample Request ID: ${sampleRequestId}`);
        
        // Add Product relation if we have a valid product ID
        if (wine.id && wine.id.length > 10) {
          // Normalize the ID format (remove dashes if needed, then add them back in correct format)
          let productId = wine.id.replace(/-/g, '');
          if (productId.length === 32) {
            productId = `${productId.slice(0,8)}-${productId.slice(8,12)}-${productId.slice(12,16)}-${productId.slice(16,20)}-${productId.slice(20)}`;
          }
          lineItemProperties['Product'] = { relation: [{ id: productId }] };
          console.log(`Adding Product relation: ${productId}`);
        }
        
        const lineItemResponse = await notionRequest('/pages', 'POST', {
          parent: { database_id: SAMPLE_LINE_ITEMS_DB_ID },
          properties: lineItemProperties
        });
        
        console.log(`Created line item: ${lineItemName}, ID: ${lineItemResponse.id}`);
      } catch (lineItemErr) {
        console.error(`Failed to create line item for ${wine.name}:`, lineItemErr.message);
        // Continue with other line items even if one fails
      }
    }
    
    // Create Pipeline entry for this producer's sample request
    const PIPELINE_DB_ID = process.env.PIPELINE_DB_ID || '267a824fc71f80a78438e035c81a9e93';
    
    try {
      // Build product relations for all wines from this producer
      const productRelations = producerWines
        .filter(w => w.id && w.id.length > 10)
        .map(w => {
          let productId = w.id.replace(/-/g, '');
          if (productId.length === 32) {
            productId = `${productId.slice(0,8)}-${productId.slice(8,12)}-${productId.slice(12,16)}-${productId.slice(16,20)}-${productId.slice(20)}`;
          }
          return { id: productId };
      ...
      // Pipeline entry name format: YYYYMMDD / State / Company / Producer
      const pipelineName = `${submissionDate} / ${state} / ${company} / ${producer}`;
      ...
      const pipelineProperties = {
        'Opportunity': { title: [{ text: { content: pipelineName } }] },
        'Status': { status: { name: 'Sample Requested' } },
        'Follow-Up Reminder': { date: { start: followUpDateStr } },
        'Sample Requests': { relation: [{ id: sampleRequestId }] }
      };
      
      // Add product relations (Product Name property)
      if (productRelations.length > 0) {
        pipelineProperties['Product Name'] = { relation: productRelations };
      }
      
      // Add state as multi-select if available
      if (state) {
        pipelineProperties['States Concerned'] = { multi_select: [{ name: state }] };
      }
      
      const pipelineResponse = await notionRequest('/pages', 'POST', {
        parent: { database_id: PIPELINE_DB_ID },
        properties: pipelineProperties
      });
      
      console.log(`Created pipeline entry: ${pipelineName}`);
    } catch (pipelineErr) {
      console.error(`Failed to create pipeline entry for ${producer}:`, pipelineErr.message);
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (resource === 'track' && resourceId && httpMethod === 'GET') {
      ...
      try {
        ...
        if (queryResponse.results && queryResponse.results.length > 0) {
          ...
          // Get existing access log (JSON array)
          let accessLog = [];
          try {
          ...
          // Add to front of log (most recent first), keep last 100
          accessLog.unshift(newEntry);
          if (accessLog.length > 100) accessLog = accessLog.slice(0, 100);
          
          // Safely truncate Access Log to 2000 chars (Notion limit)
    ...
    // ============================================
    // Price list endpoint (NO view increment here - moved to /track)
    // ============================================
    if (resource === 'price-list') {
      if (httpMethod === 'GET' && resourceId) {
        ...
        const is90PlusVersion = resourceId.endsWith('90');
        const baseListId = is90PlusVersion ? resourceId.slice(0, -2) : resourceId;
        
        console.log('=== Price List Request ===');
        console.log('resourceId:', resourceId);
        console.log('baseListId:', baseListId);
        console.log('is90PlusVersion:', is90PlusVersion);
        ...
        try {
          ...
          // NOTE: View counting moved to /api/track endpoint for better analytics
          ...
          const companyName = exportProps['Company']?.rich_text?.[0]?.plain_text || 
                             exportProps['Company Name']?.rich_text?.[0]?.plain_text || '';
          const targetMarket = exportProps['Target Market']?.select?.name || '';
          let displayName = exportProps['Name']?.title?.[0]?.plain_text || resourceId;
          
          if (is90PlusVersion) displayName = `${displayName} (90+ Scores)`;
          
          console.log('=== PRICE LIST EXPORT DEBUG ===');
          console.log('Display name:', displayName);
          console.log('Company field value:', companyName);
          console.log('Target market:', targetMarket);
          console.log('Export properties available:', Object.keys(exportProps));
          
          // Look up company address from Companies database
          let companyAddress = null;
          if (companyName) {
            const COMPANIES_DB_ID = process.env.COMPANIES_DATABASE_ID || '1baa824fc71f80da8dfcc8b23ab37c49';
            try {
              console.log('Looking up company address for:', companyName);
              console.log('Using Companies DB ID:', COMPANIES_DB_ID);
              
              // Note: Company Name is the title property in Notion Companies database
              const companySearch = await notionRequest(`/databases/${COMPANIES_DB_ID}/query`, 'POST', {
                filter: { property: 'Company Name', title: { equals: companyName } },
                page_size: 1
              });
              
              console.log('Company search results:', companySearch.results?.length || 0);
              
              if (companySearch.results && companySearch.results.length > 0) {
                const companyRecord = companySearch.results[0];
                const companyProps = companyRecord.properties;
                
                // Log available properties for debugging
                console.log('Available company properties:', Object.keys(companyProps));
                
                // Property names from Notion:
                // Street - rich_text
                // City - rich_text  
                // State Text - formula (shows state name)
                // Zip Code - rich_text
                // Country - multi_select
                
                const street = companyProps['Street']?.rich_text?.[0]?.plain_text || '';
                const city = companyProps['City']?.rich_text?.[0]?.plain_text || '';
                const state = companyProps['State Text']?.formula?.string || '';
                const zip = companyProps['Zip Code']?.rich_text?.[0]?.plain_text || '';
                const country = companyProps['Country']?.multi_select?.[0]?.name || 'United States';
                
                companyAddress = { street, city, state, zip, country };
                console.log('Found company address:', companyAddress);
              } else {
                console.log('Company not found in database with name:', companyName);
              }
            } catch (companyErr) {
              console.error('Error looking up company address:', companyErr.message);
            }
          } else {
            console.log('No company name found in price list export');
          }
