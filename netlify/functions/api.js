  ...
  try {
    ...
    if (resource === 'sample-request') {
      if (httpMethod === 'POST') {
        ...
        // 2. Uncomment the email code below
        /*
        if (process.env.RESEND_API_KEY) {
          const resend = new Resend(process.env.RESEND_API_KEY);
          await resend.emails.send({
            from: 'samples@loontradingco.com',
            to: data.notifyEmails || ['john@loontradingco.com'],
            subject: `New Sample Request from ${data.contact?.company}`,
            html: `<h2>New Sample Request</h2>
              ...
              <h3>Wines (${data.wines?.length})</h3>
              <ul>${data.wines?.map(w => `<li>${w.name}</li>`).join('')}</ul>`
          });
        }
        */
