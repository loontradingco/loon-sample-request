          ...
          <div class="wine-list">
            <ul>
              ${data.wines?.map(w => {
                ...
                return `<li><strong>${fullName}</strong> - ${w.producer}</li>`;
              }).join('')}
            </ul>
          </div>
...
async function createSampleRequestInNotion(data) {
  ...
  for (const producer of producers) {
    ...
    // Format wines list for this producer - include vintage and color
    const winesShort = producerWines.map(w => {
      const details = [w.vintage, w.color].filter(Boolean).join(' ');
      return details ? `${w.name} (${details})` : w.name;
    }).join(', ');
  ...
  try {
    ...
    if (resource === 'price-list') {
      if (httpMethod === 'GET' && resourceId) {
        ...
        // Check if this is a 90+ scores version (ends with "90")
        ...
        try {
          ...
          const currentViews = exportProps['Views']?.number || 0;
          try {
            await notionRequest(`/pages/${exportRecord.id}`, 'PATCH', {
              properties: {
                'Views': { number: currentViews + 1 }
              }
            });
            console.log('View count updated:', currentViews + 1);
          } catch (viewErr) {
            console.log('Could not update view count:', viewErr.message);
          ...
          // Append 90+ indicator to display name if applicable
          if (is90PlusVersion) {
            displayName = `${displayName} (90+ Scores)`;
          }
