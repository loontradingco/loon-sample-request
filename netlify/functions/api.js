...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    // Price list endpoint - fetches wines from Notion based on price list ID
    if (resource === 'price-list') {
      if (httpMethod === 'GET' && resourceId) {
        ...
        const priceListDefs = {
          ...
          'fine-vines': {
            name: 'Fine Vines (Illinois)',
            territories: ['Illinois'],
            orDistributor: 'Fine Vines'
          },
          'la-cigale': {
            name: 'La Cigale (Texas)',
            territories: ['Texas'],
            excludeProducer: 'Maby'
          },
          'mn': {
            name: 'Minnesota',
            territories: ['Minnesota']
          },
          'mx': {
            name: 'Mexico',
            territories: ['Mexico']
        ...
        const def = priceListDefs[resourceId];
        if (!def) {
          return {
            statusCode: 404,
            headers,
            body: JSON.stringify({ error: 'Price list not found' })
          };
        }
        ...
        try {
          ...
          if (def.territories) {
            filters.and.push({
              ...
              multi_select: { contains: def.territories[0] }
            });
          }
          ...
          if (def.targetMarket) {
            filters.and.push({
              ...
              multi_select: { contains: def.targetMarket }
            });
          }
          ...
          while (hasMore && products.length < 1000) {
            ...
            for (const page of response.results) {
              ...
              // Exclude producer filter
              if (def.excludeProducer && producer.toLowerCase().includes(def.excludeProducer.toLowerCase())) {
                include = false;
              }
              
              // Or distributor filter - check Current Distributors field
              if (def.orDistributor) {
                const distributors = props['Current Distributors']?.multi_select?.map(d => d.name) || [];
                if (!distributors.some(d => d.toLowerCase().includes(def.orDistributor.toLowerCase()))) {
                  include = false;
                }
              }
